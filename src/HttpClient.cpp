...
UrlComponents parseUrl(const std::string& url)
{
    ...
    if (std::regex_match(url, urlMatchResult, urlRegex)) {
        return {
            urlMatchResult[1].str(),
            urlMatchResult[2].str(),
...
net::awaitable<std::pair<std::size_t, std::string>> HttpClient::Get(const std::string& url)
{
    try {
        ...
        using tcp_stream =
            typename beast::tcp_stream::rebind_executor<executor_with_default>::other;
        ...
        auto const results = co_await resolver.async_resolve(host, protocol, net::use_awaitable);
        if (protocol == "https") {
            ...
            beast::tcp_stream stream(net::use_awaitable.as_default_on(beast::tcp_stream(executor)));
            co_await beast::get_lowest_layer(stream).async_connect(results, net::use_awaitable);
            co_await http::async_write(stream, req, net::use_awaitable);
            co_await http::async_read(stream, buffer, res, net::use_awaitable);
        }
