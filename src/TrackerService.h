...
public:
  ...
  discoverPeers(const torrent::MetaData &torrent, bool compact = true) {
    ...
    snprintf(url_buffer, 2048,
             "%s?info_hash=%s&peer_id=%s&port=%lld&uploaded=%lld&downloaded=%"
             "lld&left=%lld&compact=%d",
             torrent.announce.c_str(), url_encoded_hash.c_str(),
             torrent.peer_id.c_str(), torrent.port, torrent.uploaded,
             torrent.downloaded, torrent.left, compact ? 1 : 0);
