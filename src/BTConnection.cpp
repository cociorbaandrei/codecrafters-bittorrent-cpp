...
void BTConnection::dispatchMessage(const BTMessage& message)
{
    ...
    case BTMessageType::Piece: {
        ...
        //		spdlog::debug("Piece Index: {:0}, Begin: {:1}, Size: {:2}", pieceIndex,
        // begin, piece_data.size());
        ...
        // openReq->on<uvw::fs_event>([openReq, pieceIndex, piece_data, begin, blockSize,
        // this](const auto& event, auto& req) {
        ...
        // 		std::int64_t offset = pieceIndex * piece_length + begin; // Offset in bytes,
        // where to start writing 		req.write(( char*)piece_data.data(),
        // piece_data.size(), pieceIndex
        // * piece_length + begin);
        ...
        // uvw::fs_req::fs_type::WRITE"); 		req.close(); // Close the file once writing
        // is done
        // 	}
        // });
        // auto flags = uvw::file_req::file_open_flags::CREAT |
    ...
    default:
        ...
        prettyPrintHex(message.payload);
        // std::cerr << "Unknown message type received: " << static_cast<int>(message.type) <<
        // std::endl;
        break;
    }
}
...
void BTConnection::requestDownload(size_t piece_index, size_t blockIndex)
{
    ...
    for (int block = 0; block < this->pieces[piece_index].blocks.size(); ++block) {
        ...
        int length = this->pieces.size() - 1 == piece_index
                && this->pieces[piece_index].blocks.size() - 1 == block
...
void BTConnection::onBlockReceived(
    size_t pieceIndex, size_t blockIndex, const std::vector<uint8_t>& data)
{
    ...
    if (piece.isComplete()) {
        ...
        // m_tcp_handle->write(data, 5);
        spdlog::info(
            "Piece {0} downloaded to {1} {2} {3}",
            pieceIndex,
            request_download_name,
            pieceIndex,
            blockIndex);
...
void BTConnection::writePieceToFile(size_t pieceIndex, const Piece& piece)
{
    ...
    std::ofstream file(
        m_metadata.info.name + "v_2", std::ios::binary | std::ios::app | std::ios::out);
    if (!file.is_open()) {
